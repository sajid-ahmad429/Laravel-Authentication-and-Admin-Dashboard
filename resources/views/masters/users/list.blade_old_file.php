...
<!-- Custom Style for Materialize Error State matching the image -->
<style>
    .errborder,
    .is-invalid {
...
<div class="content-wrapper">
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasAddUser"
                ...
                <div class="offcanvas-body mx-0 flex-grow-0 h-100">
                    <form class="add-new-user pt-0" id="addNewUserForm" action="{{ url($roleName . '/add-new-users') }}"
                        ...
                        <button type="reset" class="btn btn-outline-secondary" data-bs-dismiss="offcanvas" onclick="clearFormErrors()">Cancel</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function validate_form(form_name) {
            ...
            $('#' + form_name + ' input,#' + form_name + ' select,#' + form_name + ' textarea,#' + form_name +
                ' checkbox,#' + form_name + ' date').each(function(index) {
                ...
                if (input.hasClass('required') && $.trim(input.val()) == '') {
                    flag2 = 1;
                    flag = 1;
                    // Format error message cleanly like the Materialize theme design
                    var placeholderText = input.attr('placeholder') || 'field';
                    input.after("<div class='err_warning'>Please enter your " + placeholderText.toLowerCase() +
                        "</div>");
                ...
                if (input.attr('data-bind') && flag2 == 0) {
                    ...
                    if ($.trim(str) != '') {
                        if (!temp1.test(str)) {
                            flag1 = 1;
                            var placeholderText = input.attr('placeholder') || 'field';
                            input.after("<div class='err_warning'>Please enter a valid " + placeholderText
                                .toLowerCase() + "</div>");
                            $(input).addClass("errborder");
                        }
                    }
                ...
                if (input.attr('type') == 'file') {
                    ...
                    if (input.hasClass('required')) {
                        if (file_selected.length == 0 || file_selected.length > 5) {
                            flag1 = 1;
                            $(input).after("<div class='err_warning'>Please select a valid file</div>");
                            $(input).addClass("errborder");
                        }
                    }
                }

                if (input.attr('type') == 'checkbox' && !$('input[name="' + input.attr('name') + '"]:checked')
                    .length) {
                    flag2 = 1;
                    flag = 1;
                    $(input).after("<div class='err_warning'>This field is required</div>");
                    $(input).addClass("errborder");
                }
                ...
                if (input.hasClass('select2') && input.hasClass('select2Required')) {
                    ...
                    if (inputValue === '' || parseInt(inputValue) === 0) {
                        flag2 = 1;
                        flag = 1;
                        var errorMessage = "<div class='err_warning'>Please select an option</div>";

                        if (errorContainer.length > 0) {
                            errorContainer.html(errorMessage);
                        } else {
                            $("#" + getId).parent().append("<div class='error-container'>" + errorMessage +
                                "</div>");
                        }
                        ...
                        if (result.status != 1) {
                            if (result.validation) {
                                if (result.validation.userEmail) {
                                    $('#add-user-email').after(
                                        "<div class='err_warning'>" + result.validation.userEmail[0] +
                                        "</div>");
                                    $('#add-user-email').addClass("errborder");
                                }

                                if (result.validation.userContact) {
                                    $('#add-user-contact').after(
                                        "<div class='err_warning'>" + result.validation.userContact[0] +
                                        "</div>");
                            ...
                            var myOffcanvas = document.getElementById('offcanvasAddUser');
                            var bsOffcanvas = bootstrap.Offcanvas.getInstance(myOffcanvas) || new bootstrap
        ...
        function editRecord(id) {
            $.ajax({
                async: false,
                dataType: 'json',
                url: "{{ url($roleName . '/users/get-details') }}",
                method: 'POST',
                data: {
                    id: id,
        ...
        // Errors clear karne ka function
        function clearFormErrors() {
            // Saare error warning text remove karein
            $(".err_warning").each(function() {
                $(this).remove();
            });
            // Inputs se error borders remove karein
            $(".errborder").each(function() {
                $(this).removeClass("errborder");
            });
            // Agar Select2 error container ho toh use bhi clear karein
            $(".error-container").each(function() {
                $(this).html("");
            });
        }

        // Bootstrap Offcanvas close hone par automatically errors clear ho jayenge
        document.getElementById('offcanvasAddUser').addEventListener('hidden.bs.offcanvas', function() {
            clearFormErrors();
            // Form fields ko reset bhi kar sakte hain agar zaroorat ho
