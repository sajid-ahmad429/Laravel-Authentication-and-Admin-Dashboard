            ...
            <div class="w-px-400 mx-auto pt-5 pt-lg-0">
                ...
                @if (session('showResend'))
                    <div class="alert alert-info" role="alert">
                        ...
                        <form action="{{ route('resend.activation') }}" method="POST" class="d-flex gap-2">
                            @csrf
                            <input type="hidden" name="email" value="{{ session('resendEmail', old('email')) }}">
                            <button type="submit" class="btn btn-sm btn-label-info">Resend activation link</button>
                        </form>
                    </div>
                @endif

                <form id="formAuthentication" class="mb-3" action="{{ route('login.attempt') }}" method="POST">
                    @csrf
                    <div class="form-floating form-floating-outline mb-3">
                        <input type="email" class="form-control @error('email') is-invalid @enderror"
                            id="email" name="email" placeholder="Enter your email" autocomplete="username"
                    ...
                    <div class="mb-3 d-flex justify-content-between">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" id="remember-me" name="rememberme" value="1" />
